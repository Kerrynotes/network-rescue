...
```text
Install-NetworkRescue.bat
```

普通安装写入当前用户的 `%LOCALAPPDATA%\KerryNetworkRescue`，不需要管理员权限。安装后会创建开机启动快捷方式，并显示托盘盾牌图标。

如果你在托盘菜单中选择了“退出断网急救”，不需要重新安装。双击压缩包或安装目录中的 `Start-NetworkRescue.bat`，即可恢复托盘监控。

所有 `.bat` 入口均使用英文文件名、纯 ASCII 内容和 Windows CRLF 换行，避免不同系统区域设置或解压工具对中文批处理编码解析不一致。程序界面和说明文档仍保持中文。

只有停止受保护服务、清理高权限 TUN 或机器级代理设置时才需要管理员权限：

- 未安装 Helper：普通权限步骤完成后，仍有残留才弹一次 UAC；
- 安装 Helper：双击 `Install-Privileged-Helper.bat`，安装时确认一次 UAC，以后高权限动作仍需产品内确认，但不再弹 UAC。
...
后台会统一监控当前实际接管网络的路径，而不是只盯着龙猫云：系统代理开启时检查当前 Owner 的本地端口和代理联网；TUN 或普通直连时同时区分本地网络故障与代理路径故障。启动后先执行不含联网探测的快速扫描，优先识别系统代理、端口和后台 Core；随后再补充完整联网检测。确认代理线路故障后会记录断连和恢复事件。双击 `Open-Connection-Logs.bat` 可打开 `代理连接记录.csv`。

如果一次完整扫描超过 45 秒，程序会中止本次扫描并自动重试；托盘菜单会明确显示“检查失败（点此查看）”，不再无限停留在“正在检查”。

### 网络状态

查看当前系统代理 Owner、地址、本地代理端口归属、后台 Core 归属、TUN、诊断状态、直连和代理联网结果。基础状态使用“项目 / 当前状态”两列表格展示；诊断说明、风险项和建议单独分段并按编号列出。发现风险时提供“退出全部代理并恢复普通网络”和“暂不处理”两个选项。选择修复后仍会显示完整清场对象并再次确认，不会直接无提示结束客户端。
...
- 断连判断跟随当前 Windows 系统代理、应用代理或 TUN/直连路径，不会因为某个未接管网络的客户端没有连接而误报断连。
...
- 监控日志、修复动作日志和 Helper 日志：单文件最大 2 MB，保留 3 份归档；
- 结构化事件：单文件最大 5 MB，保留 3 份归档；
- 代理连接记录：保留最近 180 天，同时最多保留 5000 条；
- `latest-state.json` 和 `last-repair-result.json` 每次覆盖写入，只保留最新状态。
...
1. `Install-NetworkRescue.bat`：首次安装或升级；
2. `Start-NetworkRescue.bat`：手动退出后重新启动；
3. `Open-Connection-Logs.bat`：打开当前代理路径的断连历史；
4. `Uninstall-NetworkRescue.bat`：移除开机启动并停止监控。

其余 `.ps1`、`client_adapters.json` 和 `helper` 文件夹都是程序运行所需组件，不能单独删除。`Install-Privileged-Helper.bat` 与 `Uninstall-Privileged-Helper.bat` 是可选的管理员功能，仅在你选择安装或移除 Helper 时使用。
