...
后台会统一监控当前实际接管网络的路径，而不是只盯着龙猫云：系统代理开启时检查当前 Owner 的本地端口和代理联网；TUN 或普通直连时同时区分本地网络故障与代理路径故障。确认代理线路故障后会记录断连和恢复事件。双击 `查看代理连接记录.bat` 可打开 `代理连接记录.csv`。
